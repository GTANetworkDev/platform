...
namespace GTA
{
	...
	void Vehicle::SetWheelRotation(int index, float value)
	{
		auto entityAddr = Native::MemoryAccess::GetAddressOfEntity(Handle);

		unsigned long long wheelPtr = *reinterpret_cast<unsigned long long *>(entityAddr + index * 8);

		if (wheelPtr)
			*reinterpret_cast<float *>(wheelPtr + 0x164) = value;
	}
